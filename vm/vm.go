...
func (vm *VM) executeArrayIndex(array, index object.Object) error {
	...
	maxOuter := int64(len(arrayObject.Elements))
	if i < 0 || i >= maxOuter {
		return vm.push(Null)
	}
...
func (vm *VM) executeHashIndex(hashMap, index object.Object) error {
	hashObject := hashMap.(*object.Hash)
	key, ok := index.(object.Hashable)
	if !ok {
		return fmt.Errorf("unusable as has key: %s", index.Type())
	}
...
func (vm *VM) executeBinaryStringOperation(
	...
	return vm.push(&object.String{Value: str})
}
...
func (vm *VM) currentFrame() *Frame {
	return vm.frames[vm.nextFramesIndex-1]
}

func (vm *VM) pushFrame(f *Frame) {
	vm.frames[vm.nextFramesIndex] = f
	vm.nextFramesIndex++
}

func (vm *VM) popFrame() *Frame {
	vm.nextFramesIndex--
	return vm.frames[vm.nextFramesIndex]
}
